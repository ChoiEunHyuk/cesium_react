...
const SLOPE_COLORS: SlopeColorRange[] = [
...
// 경사도에 따른 색상 반환
const getSlopeColor = (slopeDegrees: number): Cesium.Color => {
...
export interface UseSlopeAnalysisReturn {
  isAnalyzing: boolean
  ...
  setGridResolution: React.Dispatch<React.SetStateAction<number>>
  startHoverMode: () => void
  stopHoverMode: () => void
...
export function useSlopeAnalysis(viewerRef: CesiumViewerRef): UseSlopeAnalysisReturn {
  const [isAnalyzing, setIsAnalyzing] = useState(false)
  const [isSelectingArea, setIsSelectingArea] = useState(false)
  const [hoverSlope, setHoverSlope] = useState<SlopeResult | null>(null)
  ...
  const analysisEntitiesRef = useRef<Cesium.Entity[]>([])
  const polygonEntityRef = useRef<Cesium.Entity | null>(null)

  // 특정 지점의 경사도 계산
  const calculateSlopeAtPoint = useCallback(async (longitude: number, latitude: number, cellSize = 10): Promise<SlopeResult | null> => {
  ...
  const startHoverMode = useCallback(() => {
    ...
    handler.setInputAction(async (movement: Cesium.ScreenSpaceEventHandler.MotionEvent) => {
      const now = Date.now()
  ...
  const isPointInPolygon = (x: number, y: number, polyX: number[], polyY: number[]): boolean => {
    let inside = false
  ...
  // 결과 시각화
  const visualizeResults = useCallback((results: SlopeResult[]) => {
    if (!viewerRef.current) return
  ...
  const analyzeArea = useCallback(async () => {
    ...
    for (let i = 0; i < gridPoints.length; i += batchSize) {
      ...
      results.push(...batchResults.filter((r): r is SlopeResult => r !== null))
  ...
  }, [viewerRef, gridResolution, calculateSlopeAtPoint, visualizeResults])
  ...
  }, [viewerRef, clearAnalysisResults, updatePolygonPreview, finishAreaSelection])

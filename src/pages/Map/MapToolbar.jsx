...
function MapToolbar({ viewerRef }) {
  ...
  const [markerFormData, setMarkerFormData] = useState({ title: '', description: '' })
   ...
   * Refs - 거리 측정용
   * ======================================================================== */
  const measureHandlerRef = useRef(null)    // 마우스 이벤트 핸들러
  const measurePointsRef = useRef([])       // 측정 포인트 좌표 배열
  const measureEntitiesRef = useRef([])     // 지도에 표시된 엔티티(점, 선, 라벨)

  /* ========================================================================
   * Refs - 면적 측정용
   * ======================================================================== */
  const areaHandlerRef = useRef(null)       // 마우스 이벤트 핸들러
  const areaPointsRef = useRef([])          // 폴리곤 꼭지점 좌표 배열
  const areaEntitiesRef = useRef([])        // 지도에 표시된 엔티티(점, 선)
  const areaPolygonRef = useRef(null)       // 폴리곤 엔티티

  /* ========================================================================
   * Refs - 마커 관련
   * ======================================================================== */
  const markerHandlerRef = useRef(null)     // 마커 클릭 이벤트 핸들러
  const markersRef = useRef([])             // 마커 엔티티 배열
   ...
   * ======================================================================== */
  const addMeasurePoint = useCallback((cartesian) => {
   ...
   * ======================================================================== */
  const updateAreaPolygon = useCallback((positions) => {
    if (!viewerRef.current) return
  ...
  const addAreaPoint = useCallback((cartesian) => {
    if (!viewerRef.current || !cartesian) return
    ...
    clickHandler.setInputAction((click) => {
      ...
      if (Cesium.defined(pickedObject) && pickedObject.id) {
        ...
        if (entity.properties && entity.properties.isMarker) {
          setSelectedMarker({
            title: entity.properties.title.getValue(),
            ...
            <div className="marker-modal-content">
              <div className="marker-form-group">
                ...
                <input
                  ...
                  onChange={(e) => setMarkerFormData(prev => ({ ...prev, title: e.target.value }))}
                  autoFocus
                />
              </div>
              <div className="marker-form-group">
                ...
                <textarea
                  ...
                  onChange={(e) => setMarkerFormData(prev => ({ ...prev, description: e.target.value }))}
                />
              </div>
            </div>
